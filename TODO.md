...
- [x] Remove Copier control artifacts (`copier.yml`, `.copier-answers.yml`) from generated projects to avoid implicit template-update tracking.
...
- [x] Raise template SDK minimum to `wordlift-sdk>=6.15.1,<7.0.0` and align docs/tests to the current SDK baseline.
- [x] Lower template Python baseline to `>=3.12` and align GitHub Actions workflows on cached `uv` setup.
...
- [x] Update template SDK baseline to `wordlift-sdk>=8.0.17,<9.0.0`; keep generated workflow on `wordlift/graph-sync@v6` and set `worai_version` explicitly to `6.17.19`.
