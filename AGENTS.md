...
- Workflow contract is profile-based (no country input), in `.github/workflows/graph-sync.yml`; the workflow pins its `worai` version and graph KPI calculation is enabled by default.
- Runtime config template is `worai.toml.jinja` (rendered output: `worai.toml`).
- Runtime template follows SDK v8.4.1 cloud-flow contract (`ingest_source`, `ingest_loader`, `ingest_timeout_ms`; no `web_page_import_*` fallback keys).
